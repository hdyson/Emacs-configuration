...
Pycharm has some pretty ligature support.  Let's see if we can do similar in the one true editor.  The built-in ~prettify-symbols~ mode looks to be a good place to start.  From [[http://www.aliquote.org/post/enliven-your-emacs/][this blog]], we have some basic support:
#+begin_src emacs-lisp
 ...
 (lambda ()
   (mapc (lambda (pair) (push pair prettify-symbols-alist))
	    ...
	    ("lambda" . 955)
            ("->" . 8594)
            ("=>" . 8658)
;; I don't find the symbol for ~map~ to be more readable, so let's disable it:
;;            ("map" . 8614)
            ("<=" . 8804)
            (">=" . 8805)
            ("<-" . 8592)
