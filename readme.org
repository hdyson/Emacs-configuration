...
#+BEGIN_SRC emacs-lisp
  ...
  (backup-directory-alist '(("." . "~/.emacs.backups")))  ;; backup files to stand alone directory
  (custom-enabled-themes '(tango-dark))
  (revert-without-query '(".*\\.err" ".*\\.out"))
  (switch-to-buffer-in-dedicated-window 'prompt)  ;; better compilation window handling - useful for qmk
  (tab-always-indent 'complete)  ;; attempt to indent, if that doesn't work trigger completion.
  (tab-stop-list '(8 16)))  ;; make tabs stupidly wide to better identify accidental tabs
